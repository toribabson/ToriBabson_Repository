...
Bayesian methods do seem more intuitive and I don't know how any biologist could fail to appreciate Bayesian statistics for the ability to calculate variable parameter estimates. When studying biological processes there will never be one "true" value when life depends so heavily on variation to exist.  A Bayesian 95% CI provides a range in which 95% of the parameter values are contained instead of a range in which there is a 95% chance that the single "true" value is contained. The Bayesian technique gives answers that seem more informative than the frequentist. Another aspect of Bayesian methods that I like is the prior. When starting from scratch a weak prior can be used and avoid any significant influence on the posterior. Human beings learn by making connections between past and new experiences. If we lived without the ability to learn from past experiences, humans would be as functional as infants with only natural instincts like breathing. Thinking logically, the prior is included because that is how we are able to develop intelligent beings, so to learn more from our statistical tests of course we would use any prior information. If a Bayesian and a frequentist learn how to walk and want to learn how to run, only the bayesian would succeed by remembering how to walk first. 
...
seals <- read_csv('~/Dropbox/Data_Science/Labs/Oct13/data/06/17e8ShrinkingSeals Trites 1996.csv')
set.seed(607)

##### SAMPLE SIZE, INTERCEPT, SLOPE (and alpha) for varying parameters

# simulation data frame with the parameters and information that varies
# SAMPLE SIZES, SLOPES
...
# expand to have a certain number of simulations for eatch sample size
simPopN <- simPopN %>%
  group_by(slope, intercept, sigma, n, alpha) %>%
  expand(reps = 1:n) %>%
  ungroup()

# simulate each 500 of times
simPopN <- simPopN %>%
  crossing(sim = 1:500)


## simulate data -just add in fitted values
# alsosimulate random draws of ages
simPopN <- simPopN %>%
...
# first generate a lot of fit models
fits <- simPopN %>%
  group_by(slope, intercept, sigma, n, sim, alpha) %>%
  nest()

# iterate over diff levels of grouped data frames and perform some funcion
# we will fit a model, get its coefficients using *broom*

fits <- fits %>%
    mutate(mod = map(data, ~lm(length.cm ~ age.days, data=.))) %>%
    mutate(coefs = map(mod, ~tidy(.)))

#cleanup - unnest, which takes list-columns from above, and expands them out as into full data frames that get slotted back into our original data frame

#filter slope coefficient.
fits <- fits %>%
  unnest(coefs) %>%
  ungroup() %>%
